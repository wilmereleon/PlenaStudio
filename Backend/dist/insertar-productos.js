...
async function insertarProductos() {
    try {
        ...
        for (const producto of productos) {
            try {
                await db_1.db.query("INSERT INTO producto (id_producto, nombre, precio, imagen_url, descripcion, stock) VALUES (?, ?, ?, ?, ?, ?)", [producto.id_producto, producto.nombre, producto.precio, producto.imagen_url, producto.nombre, 100]);
        ...
        const [productos_insertados] = await db_1.db.query("SELECT COUNT(*) as total FROM producto");
        console.log(`\n📊 Total de productos en BD: ${productos_insertados[0].total}`);
    }
    catch (error) {
