...
async function testearCarrito() {
  try {
    ...
    await db.query("DELETE FROM carrito_item WHERE id_carrito IN (SELECT id_carrito FROM carrito WHERE id_usuario = 1)");
    await db.query("DELETE FROM carrito WHERE id_usuario = 1");
    ...
    // Crear carrito
    await db.query("INSERT INTO carrito (id_usuario, fecha_creacion) VALUES (1, NOW())");
    const [carritos] = await db.query("SELECT * FROM carrito WHERE id_usuario = 1");
    const carrito = (carritos as any[])[0];
    console.log("Carrito creado:", carrito);
    
    // Agregar item al carrito
    await db.query(
      "INSERT INTO carrito_item (id_carrito, id_producto, cantidad, precio_unitario) VALUES (?, ?, ?, ?)",
      [carrito.id_carrito, 1, 2, 28000]
    );
    ...
    await db.query("DELETE FROM carrito_item WHERE id_carrito = ?", [carrito.id_carrito]);
    await db.query("DELETE FROM carrito WHERE id_carrito = ?", [carrito.id_carrito]);
    console.log("✅ Carrito limpiado tras logout");
    
    // 5. Verificar que se limpió
    console.log("\n5. Verificando que el carrito está vacío...");
    const [carritosFinal] = await db.query("SELECT * FROM carrito WHERE id_usuario = 1");
    const [itemsFinal] = await db.query("SELECT * FROM carrito_item");
    
    console.log("Carritos restantes para usuario 1:", carritosFinal);
    console.log("Items totales en BD:", itemsFinal);
    
    if ((carritosFinal as any[]).length === 0) {
