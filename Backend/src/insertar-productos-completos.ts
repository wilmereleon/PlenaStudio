  ...
  try {
    ...
    for (const producto of productosCompletos) {
      try {
        ...
        await db.query(
          `INSERT INTO producto (id_producto, nombre, descripcion, precio, imagen_url, stock, fecha_creacion) 
           VALUES (?, ?, ?, ?, ?, ?, NOW())`,
          [
    ...
    const totalEnBD = (totalResult as any[])[0].total;
...
export { insertarProductosCompletos, productosCompletos };
