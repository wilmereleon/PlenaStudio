...
export const cartService = {
   ...
   */  async getCartByUserId(userId: string): Promise<CartItem[]> {
    try {
      ...
      const [carts] = await db.query("SELECT * FROM carrito WHERE id_usuario = ?", [userId]);
      let cart = (carts as any[])[0];
      
      if (!cart) {
        // Crear carrito si no existe
        await db.query("INSERT INTO carrito (id_usuario, fecha_creacion) VALUES (?, NOW())", [userId]);
        const [newCarts] = await db.query("SELECT * FROM carrito WHERE id_usuario = ?", [userId]);
    ...
    try {
      ...
      for (const item of items) {
        ...
        await connection.query(
          ...
          [cart.id_carrito, item.productId, item.cantidad, precioUnitario]
        );
      }
   ...
   */  async addToCart(userId: string, productId: string, cantidad: number): Promise<CartItem[]> {
    try {
      ...
      // Obtener precio actual del producto
      const [productos] = await db.query("SELECT precio FROM producto WHERE id_producto = ?", [productIdNum]);
      const producto = (productos as any[])[0];
      if (!producto) {
      ...
      const [existingItems] = await db.query(
        "SELECT * FROM carrito_item WHERE id_carrito = ? AND id_producto = ?", 
        [cart.id_carrito, productIdNum]
      );

      if ((existingItems as any[]).length > 0) {
        // Actualizar cantidad si ya existe
        await db.query(
          "UPDATE carrito_item SET cantidad = cantidad + ? WHERE id_carrito = ? AND id_producto = ?",
          [cantidad, cart.id_carrito, productIdNum]
        );
      } else {
        // Insertar nuevo item
        await db.query(
          "INSERT INTO carrito_item (id_carrito, id_producto, cantidad, precio_unitario) VALUES (?, ?, ?, ?)",
          [cart.id_carrito, productIdNum, cantidad, producto.precio]
        );
      }
   ...
   */  async removeFromCart(userId: string, productId: string): Promise<CartItem[]> {
    try {
      ...
      await db.query(
        "DELETE FROM carrito_item WHERE id_carrito = ? AND id_producto = ?", 
        [cart.id_carrito, productIdNum]
      );
   ...
   */  async clearCart(userId: string): Promise<void> {
    try {
      ...
      await db.query("DELETE FROM carrito_item WHERE id_carrito = ?", [cart.id_carrito]);
    } catch (error) {
   ...
   */
  async updateCartItemQuantity(userId: string, productId: string, cantidad: number): Promise<void> {
    try {
      const productIdNum = parseInt(productId);
      if (isNaN(productIdNum)) {
        throw new Error("ID de producto inválido");
      ...
      await db.query(
        "UPDATE carrito_item SET cantidad = ? WHERE id_carrito = ? AND id_producto = ?",
        [cantidad, cart.id_carrito, productIdNum]
      );
    } catch (error) {
   ...
   */
  async syncCartOnLogin(userId: string, localCartItems: CartItem[] = []): Promise<CartItem[]> {
    const connection = await db.getConnection();
